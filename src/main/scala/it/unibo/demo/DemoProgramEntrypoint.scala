...
import it.unibo.demo.robot.{Actuation, RobotUpdate, WaveRobot, ThymioRobot}
...
object DemoProgramEntrypoint extends JFXApp3 {
  ...
  private val aggregateProgram: AggregateProgram = RobotRotation()
  ...
  private val robots = List(ThymioRobot("", 5))//read[Seq[WaveRobot]](data).toList
  ...
  private val list = thymioRobots.map(_.id)
  ...
  private val update = RobotUpdate(thymioRobots, 0.25)

  override def start(): Unit =
    val aggregateOrchestrator =
      AggregateOrchestrator[Position, Info, Actuation](thymioRobots.map(_.id).toSet, aggregateProgram)
